...
export const LandingScreen: React.FC<LandingScreenProps> = ({ onOpenPrivacy }) => {
  ...
  const handleNextCard = () => {
    ...
    const activeEl = deckRef.current?.querySelector(`[data-card-index="${topIndex}"]`);

    if (activeEl) {
      // Smooth tactile fling animation
      gsap.to(activeEl, {
        x: 140,
        y: -30,
        rotation: 16,
  ...
  return (
    <div ref={containerRef} className="py-10 sm:py-16 px-4 max-w-6xl mx-auto flex flex-col items-center">
      {/* Hero Headline */}
      ...
      <div className="w-full max-w-md my-6 flex flex-col items-center select-none">
        ...
        >
          {STRIP_CARDS.map((strip, idx) => {
            ...
            const order = (idx - topIndex + STRIP_CARDS.length) % STRIP_CARDS.length;
            const isTop = order === 0;

            // Stack transform presets
            const rotations = [0, 4, -4, 6];
            const yOffsets = [0, 8, 16, 24];
            const scales = [1, 0.96, 0.92, 0.88];
            const opacities = [1, 0.9, 0.8, 0.65];

            const rot = rotations[order] || 0;
            const yOff = yOffsets[order] || 0;
            const sc = scales[order] || 0.88;
            const op = opacities[order] || 0.5;
            const zIdx = 10 - order;
            ...
            return (
              <div
                key={strip.id}
                data-card-index={idx}
                className="absolute inset-0 bg-white dark:bg-stone-900 rounded-2xl p-2.5 sm:p-3 border border-stone-200 dark:border-stone-700 shadow-md transition-all duration-300 flex flex-col justify-between overflow-hidden"
                style={{
                  transform: `translateY(${yOff}px) rotate(${rot}deg) scale(${sc})`,
                  opacity: op,
                  zIndex: zIdx,
                  pointerEvents: isTop ? 'auto' : 'none',
                }}
              >
                {/* Strip Cutout Photos - constrained so they never overflow */}
